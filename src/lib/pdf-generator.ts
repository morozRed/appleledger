...
function addSummarySection(
  ...
  doc.text(
    `${report.summary.byCurrency.length} currency${report.summary.byCurrency.length !== 1 ? 'ies' : ''} • ${report.summary.totalTransactions} transaction${report.summary.totalTransactions !== 1 ? 's' : ''}`,
    margin,
    yPos
  );
  yPos += 10;
...
function addDisclaimer(
  ...
  const disclaimerText =
    'This document is a sales statement generated from Apple App Store financial reports. It is not an invoice issued by Apple Inc. This tool does not calculate taxes. Generated by AppStore Ledger (appstoreledger.com).';
